...
def download():
    if not os.path.exists('zipfile'): os.mkdir('zipfile')
    os.chdir('zipfile')
    for i in range(nlab):
    ...
    for i in proj_list:
        if not os.path.exists('%s.zip'%(i)): 
            bash_shell("wget --no-check-certificate https://www-inst.eecs.berkeley.edu/~cs61a/sp22/proj/%s/%s.zip"%(i,i))
    os.chdir('..')
